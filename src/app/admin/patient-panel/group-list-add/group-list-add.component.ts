...
import { GroupDataPanelComponent } from '../group-data-panel/group-data-panel.component';
...
@Component({
  ...
  styleUrl: './group-list-add.component.css'
})
export class GroupListAddComponent implements OnInit, OnDestroy{
  ...
  sendData = () => {
    if(this.edit){
      ...
      this.saveGroupPatient();
    }
  }

  saveGroupPatient = () => {
    if(this.groupSelected?.id){
      this.saveGroupSubscription = this.patientService.saveGroupPatient(this.groupSelected.id, this.id).subscribe({
        next: (res: IsAuth) =>{
          if(res.response){
        ...
        error: (err: any) => {
          this.setMessage('El grupo no pudo ser registrado');
        }
      });
    }else{
  ...
  getGroups = (limit: number, page: number) => {
    this.groupsSubscription = this.patientService.getGroups(limit, page).subscribe({
      next: (res: Group[]) =>{
        this.groupList = res;
  ...
  getCountPatients = () => {
    this.countGroupSubscription = this.patientService.getCountGroups().subscribe({
      next: (res: SizeList) =>{
  ...
  getGroupsByAll = () => {
    this.searchGroupSubscription = this.patientService.getGroupsByAll(this.data.value.search, this.limit, this.pageSize).subscribe({
      next: (res: Group[]) =>{
        this.groupList = res;
        this.insertMonitorInList();
      },
  ...
  getNumPatients = (group: Group) => {
    if(group?.id){
      this.countPatientsSubscription = this.patientService.getCountPatientInGroup(group.id).subscribe({
        next: (res: SizeList) =>{
